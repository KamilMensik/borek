...
mrb_value sprite = MRB_GET_IV(self, "@_animated_sprite");                      \
if (mrb_nil_p(sprite)) {                                                       \
        sprite = mrb_class_new_instance(                                       \
                mrb, 1, &self,                                                 \
                RBAnimatedSpriteComponent::sprite_component_class);            \
        MRB_SET_IV(self, "@_animated_sprite", sprite);                         \
}
...
static MRB_FUNC(SetFlipX)
{
        ...
        ChangesExporter::AddAnimatedSprite(mrb, self);
...
static MRB_FUNC(Anim)
{
        GET_SPRITE_SAFE;
        return MRB_GET_IV(sprite, "@current_anim");
}

static MRB_FUNC(SetAnim)
{
        GET_SPRITE_SAFE;
        MRB_SET_IV(sprite, "@current_anim", MRB_ARG1);
        ChangesExporter::AddAnimatedSprite(mrb, self);
...
static MRB_FUNC(New)
{
        return RBNode::NewNode(mrb, RBAnimatedSprite::sprite_class, NodeType::AnimatedSprite);
}
...
NODE_MODULE_METHOD(Anim);
NODE_MODULE_METHOD(SetAnim);

void
RBAnimatedSprite::Init(RubyEngine &engine)
{
        auto& vm = engine.GetRubyVM();

        sprite_class = vm.define_class("AnimatedSpriteC", RBNode::node_class)
                ...
                .define_method("anim", Anim)
                .define_method("anim=", SetAnim, FuncArgs().Required(1));

        sprite_module = vm.define_module("AnimatedSprite")
                ...
                .define_method("anim", MAnim)
                .define_method("anim=", MSetAnim, FuncArgs().Required(1))
                .define_module_method("new", New);
}

Module RBAnimatedSprite::sprite_module;
Class RBAnimatedSprite::sprite_class;
