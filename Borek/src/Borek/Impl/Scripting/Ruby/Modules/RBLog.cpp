...
{
        BOREK_LOG_INFO("{}", mrb_string_cstr(mrb, mrb_obj_as_string(mrb, MRB_ARG1)));
        return mrb_nil_value();
}

static MRB_FUNC(Error)
{
        BOREK_LOG_ERROR("{}", mrb_string_cstr(mrb, mrb_obj_as_string(mrb, MRB_ARG1)));
        return mrb_nil_value();
}

static MRB_FUNC(Fatal)
{
        BOREK_LOG_FATAL("{}", mrb_string_cstr(mrb, mrb_obj_as_string(mrb, MRB_ARG1)));
        return mrb_nil_value();
}

static MRB_FUNC(Trace)
{
        BOREK_LOG_TRACE("{}", mrb_string_cstr(mrb, mrb_obj_as_string(mrb, MRB_ARG1)));
        return mrb_nil_value();
}

static MRB_FUNC(Warn)
{
        BOREK_LOG_WARN("{}", mrb_string_cstr(mrb, mrb_obj_as_string(mrb, MRB_ARG1)));
        return mrb_nil_value();
}
